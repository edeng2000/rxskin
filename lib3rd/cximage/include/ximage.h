...
typedef struct tag_ExifInfo {
	...
	{
		memset(Version,0,5);
		memset(CameraMake, 0, 32);
		memset(CameraModel, 0, 40);
		...
		FocalplaneXRes = 0.0f;
		FocalplaneYRes = 0.0f;
		FocalplaneUnits = 0.0f;
		Xresolution = 0.0f;
		Yresolution = 0.0f;
		ResolutionUnit = 0.0f;
		Brightness = 0.0f;
		memset(Comments,0,MAX_COMMENT + 1);
		ThumbnailPointer = NULL;
		ThumbnailSize = 0;     /* Size of thumbnail. */
		IsExif = false;
	}
	void Copy(const tag_ExifInfo& oInfo)
	{
		memcpy(Version, oInfo.Version, 5);
		memcpy(CameraMake, oInfo.CameraMake, 32);
		memcpy(CameraModel, oInfo.CameraModel, 40);
		memcpy(DateTime, oInfo.DateTime, 20);
		Height = oInfo.Height;
		Width = oInfo.Width;
		Orientation = oInfo.Orientation;
		IsColor = oInfo.IsColor;
		Process = oInfo.Process;
		FlashUsed = oInfo.FlashUsed;
		FocalLength = oInfo.FocalLength;
		ExposureTime = oInfo.ExposureTime;
		ApertureFNumber = oInfo.ApertureFNumber;
		Distance = oInfo.Distance;
		CCDWidth = oInfo.CCDWidth;
		ExposureBias = oInfo.ExposureBias;
		Whitebalance = oInfo.Whitebalance;
		MeteringMode = oInfo.MeteringMode;
		ExposureProgram = oInfo.ExposureProgram;
		ISOequivalent = oInfo.ISOequivalent;
		CompressionLevel = oInfo.CompressionLevel;
		FocalplaneXRes = oInfo.FocalplaneXRes;
		FocalplaneYRes = oInfo.FocalplaneYRes;
		FocalplaneUnits = oInfo.FocalplaneUnits;
		Xresolution = oInfo.Xresolution;
		Yresolution = oInfo.Yresolution;
		ResolutionUnit = oInfo.ResolutionUnit;
		Brightness = oInfo.Brightness;
		memcpy(Comments, oInfo.Comments, MAX_COMMENT + 1);
		if (ThumbnailPointer)
		{
			free(ThumbnailPointer);
			ThumbnailPointer = NULL;
		}
		if (oInfo.ThumbnailSize>0 && oInfo.ThumbnailPointer)
		{
			ThumbnailPointer = (uint8_t*)malloc(oInfo.ThumbnailSize);
			memcpy(ThumbnailPointer, oInfo.ThumbnailPointer, oInfo.ThumbnailSize);
		}
		ThumbnailSize = oInfo.ThumbnailSize;   
		IsExif = oInfo.IsExif;
	}
} EXIFINFO;
...
typedef struct tagCxImageInfo {
	...
	tagCxImageInfo()
	{
		dwEffWidth = 0;;			///< uint32_t aligned scan line width
		pImage = NULL;				///< THE IMAGE BITS
		pGhost = NULL;			///< if this is a ghost, pGhost points to the body
		pParent = NULL;			///< if this is a layer, pParent points to the body
		dwType = 0;				///< original image format
		memset(szLastError,0,256);	///< debugging
		nProgress = 0;			///< monitor
		nEscape = 0;			///< escape
		nBkgndIndex = 0;		///< used for GIF, PNG, MNG
		nBkgndColor.rgbBlue = nBkgndColor.rgbGreen= nBkgndColor.rgbRed = nBkgndColor.rgbReserved = 0;;		///< used for RGB transparency
		fQuality = 0.0f;			///< used for JPEG, JPEG2000 (0.0f ... 100.0f)
		nJpegScale = 0;			///< used for JPEG [ignacio]
		nFrame = 0;				///< used for TIF, GIF, MNG : actual frame
		nNumFrames = 0;			///< used for TIF, GIF, MNG : total number of frames
		dwFrameDelay = 0;		///< used for GIF, MNG
		xDPI = 0;				///< horizontal resolution
		yDPI = 0;				///< vertical resolution
		rSelectionBox = {0};		///< bounding rectangle
		nAlphaMax = 0;			///< max opacity (fade)
		bAlphaPaletteEnabled = false; ///< true if alpha values in the palette are enabled.
		bEnabled = false;			///< enables the painting functions
		xOffset = 0;
		yOffset = 0;
		for (uint32_t t=0;t< CMAX_IMAGE_FORMATS;t++)
		{
			dwCodecOpt[t] = 0;
		}
		last_c.rgbBlue = last_c.rgbGreen = last_c.rgbRed = last_c.rgbReserved = 0;;		///< used for RGB transparency
		last_c_index = 0;
		last_c_isvalid = false;
		nNumLayers = 0;
		dwFlags = 0;			///< 0x??00000 = reserved, 0x00??0000 = blend mode, 0x0000???? = layer id - user flags
		dispmeth = 0;
		bGetAllFrames = false;
		bLittleEndianHost = false;
	}
	void Copy(const tagCxImageInfo& oInfo)
	{
#if CXIMAGE_SUPPORT_EXIF
		ExifInfo.Copy(oInfo.ExifInfo);
#endif
		dwEffWidth = oInfo.dwEffWidth;;			///< uint32_t aligned scan line width
		pImage = oInfo.pImage;				///< THE IMAGE BITS
		pGhost = oInfo.pGhost;			///< if this is a ghost, pGhost points to the body
		pParent = oInfo.pParent;			///< if this is a layer, pParent points to the body
		dwType = oInfo.dwType;				///< original image format
		memset(szLastError, 0, 256);	///< debugging
		nProgress = oInfo.nProgress;			///< monitor
		nEscape = oInfo.nEscape;			///< escape
		nBkgndIndex = oInfo.nBkgndIndex;		///< used for GIF, PNG, MNG
		nBkgndColor = oInfo.nBkgndColor;;		///< used for RGB transparency
		fQuality = oInfo.fQuality;			///< used for JPEG, JPEG2000 (0.0f ... 100.0f)
		nJpegScale = oInfo.nJpegScale;			///< used for JPEG [ignacio]
		nFrame = oInfo.nFrame;				///< used for TIF, GIF, MNG : actual frame
		nNumFrames = oInfo.nNumFrames;			///< used for TIF, GIF, MNG : total number of frames
		dwFrameDelay = oInfo.dwFrameDelay;		///< used for GIF, MNG
		xDPI = oInfo.xDPI;				///< horizontal resolution
		yDPI = oInfo.yDPI;				///< vertical resolution
		rSelectionBox = oInfo.rSelectionBox;		///< bounding rectangle
		nAlphaMax = oInfo.nAlphaMax;			///< max opacity (fade)
		bAlphaPaletteEnabled = oInfo.bAlphaPaletteEnabled; ///< true if alpha values in the palette are enabled.
		bEnabled = oInfo.bEnabled;			///< enables the painting functions
		xOffset = oInfo.xOffset;
		yOffset = oInfo.yOffset;
		for (uint32_t t = 0; t < CMAX_IMAGE_FORMATS; t++)
		{
			dwCodecOpt[t] = oInfo.dwCodecOpt[t];
		}
		last_c = oInfo.last_c;				///< for GetNearestIndex optimization
		last_c_index = oInfo.last_c_index;
		last_c_isvalid = oInfo.last_c_isvalid;
		nNumLayers = oInfo.nNumLayers;
		dwFlags = oInfo.dwFlags;			///< 0x??00000 = reserved, 0x00??0000 = blend mode, 0x0000???? = layer id - user flags
		dispmeth = oInfo.dispmeth;
		bGetAllFrames = oInfo.bGetAllFrames;
		bLittleEndianHost = oInfo.bLittleEndianHost;
	}
} CXIMAGEINFO;
...
/** \addtogroup Attributes */ //@{
	...
	static BOOL     IsImage(const TCHAR* ext);
...
/** \addtogroup Painting */ //@{
	...
	int32_t	Draw(HDC hdc, int32_t x=0, int32_t y=0, int32_t cx = -1, int32_t cy = -1,int32_t desx=0,int32_t desy=0, RECT* pClipRect = 0, bool bSmooth = false, bool bFlipY = false);
	int32_t	Draw(HDC hdc, const RECT& rect,int32_t desx=0,int32_t desy=0, RECT* pClipRect=NULL, bool bSmooth = false, bool bFlipY = false);
	...
	int32_t	Stretch(HDC hdc, int32_t x=0, int32_t y=0, int32_t cx = -1, int32_t cy = -1, RECT* pClipRect = 0, bool bSmooth = false, bool bFlipY = false);
...
/** \addtogroup Decode */ //@{
	...
	bool LoadW(const TCHAR* filename, uint32_t imagetype=0);
	bool LoadA(const char * filename, uint32_t imagetype=0);
	bool LoadMemory(const char* pszData, const uint32_t& dwLen, uint32_t imagetype = 0);
