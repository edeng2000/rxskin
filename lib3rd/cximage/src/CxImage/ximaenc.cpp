...
bool CxImage::LoadW(const TCHAR * filename, uint32_t imagetype)
{
	m_strWFilename = filename;
	bool bOK = false;
	char szError[256];
	strcpy(szError,info.szLastError); 
	FILE* hFile;
	if ((hFile=_tfopen(filename,_T("rb")))==NULL)  return false;
	bOK = Decode(hFile, imagetype);
	fclose(hFile);
	if (!bOK && imagetype > 0) strcpy(info.szLastError,szError);
	return bOK;
}

bool CxImage::LoadA(const CHAR* filename, uint32_t imagetype)
{
...
bool CxImage::LoadMemory(const char* pszData, const uint32_t& dwLen, uint32_t imagetype)
{
	CxMemFile fTmp((uint8_t*)pszData, dwLen);
...
// 		DWORD dwRead = 0;
// 		BYTE* pData = (BYTE*)malloc(dwSize + 1);
// 		hFile->Read(pData, 1, dwSize);
// 		LPBYTE pImage = NULL;
// 		int n;
// 		int nX = 0;
// 		int nY = 0;
// 		pImage = stbi_load_from_memory(pData,dwSize, &nX, &nY, &n, 4);
// 		free(pData);
// 		if (!pImage) {
// 			return NULL;
// 		}
...
// 		head.biSizeImage = nX * nY * 4;
// 
// 		bool bAlphaChannel = false;
// 		pDib = malloc(head.biSizeImage);
// 		LPBYTE pDest = (LPBYTE)pDib;
