...
#endif //_WIN32_WCE
////////////////////////////////////////////////////////////////////////////////
int32_t CxImage::Draw(HDC hdc, const RECT& rect,int32_t desx,int32_t desy, RECT* pClipRect, bool bSmooth, bool bFlipY)
{
	return Draw(hdc, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,desx,desy, pClipRect,bSmooth, bFlipY);
}
////////////////////////////////////////////////////////////////////////////////
/**
					  ...
					  bool bFlipY)
{
	...
	if (!(bTransparent || bAlpha || info.bAlphaPaletteEnabled)){
		if (/*cx==head.biWidth && cy==head.biHeight*/TRUE){ //NORMAL
#if !defined (_WIN32_WCE)
			SetStretchBltMode(hdc,COLORONCOLOR);
#endif
			if (bFlipY){
				StretchDIBits(hdc, x, y+cy-1,
					cx, -cy, desx, desy, cx, cy,
				...
				int32_t nTotalWidth = ((((24 * destw) + 31) / 32) * 4);
				CHAR* szData = (CHAR*)malloc(nTotalWidth*desth);
				for (INT32 h = 0; h < desth; h++)
				{
					for(INT32 w = 0 ; w < destw; w ++)
					{
						szData[(desth-1-h)*nTotalWidth+3*w] = info.pImage[info.dwEffWidth*(head.biHeight-(desy+h)-1)+3 * (desx+w)];
						szData[(desth-1-h)*nTotalWidth+3*w+1] = info.pImage[info.dwEffWidth*(head.biHeight-(desy+h)-1)+3 * (desx+w)+1];
						szData[(desth-1-h)*nTotalWidth+3*w+2] = info.pImage[info.dwEffWidth*(head.biHeight-(desy+h)-1)+3 * (desx+w)+2];
					}
				}
 				StretchDIBits(hdc, paintbox.left,paintbox.top,destw,desth, 0, 0, destw,desth,
 					szData,(BITMAPINFO*)&bmInfo,DIB_RGB_COLORS,SRCCOPY);
				free(szData);
			}
		} else { //STRETCH
			//pixel informations
			...
			if (pbase){
				...
				float fx=1.0;//(float)head.biWidth/(float)cx;
				float fy=1.0;//(float)head.biHeight/(float)cy;
			...
			SetDIBitsToDevice(hdc,paintbox.left,paintbox.top,destw,desth,desx,desy,0,desth,pbase,&bmInfo,0);
	...
	//////////////////////////////////////////////////////////////////
		...
		if (pbase){
			...
			if (FALSE){
				...
				for(yy=0;yy<desth;yy++){
					dy = desy;//head.biHeight-(ymax-yy-y)*fy;
					sy = max(0L,(int32_t)floor(dy));
				...
				//NORMAL
				iy= head.biHeight - desy - desth; //head.biHeight-ymax+y;
...
// 		}
		StretchDIBits(hdc, paintbox.left,paintbox.top,destw,desth, 0,0, destw,desth,
			pbase,(BITMAPINFO*)&bmInfo,DIB_RGB_COLORS,SRCCOPY);
/*		free(szData);*/
...
int32_t CxImage::Stretch(HDC hdc, int32_t x, int32_t y, int32_t cx, int32_t cy, RECT* pClipRect, bool bSmooth, bool bFlipY)
{
	if((pDib==0)||(hdc==0)||(cx==0)||(cy==0)||(!info.bEnabled)) return 0;
	...
	if((pDib)&&(hdc)&&(rc)) {
		...
		for (y = 0 ; y < h ; y += by){
			...
			for (x = 0 ; x < w ; x += z){
				...
				Draw(hdc,rc->left + x, rc->top + y,-1,-1,0,0,&r);
			}
		}
		return 1;
	}
...
int32_t CxImage::LayerDrawAll(HDC hdc, int32_t x, int32_t y, int32_t cx, int32_t cy, RECT* pClipRect, bool bSmooth)
{
	...
	while(pLayer=GetLayer(n++)){
		if (pLayer->Draw(hdc,x+pLayer->info.xOffset,y+pLayer->info.yOffset,cx,cy,0,0,pClipRect,bSmooth)==0)
			return 0;
