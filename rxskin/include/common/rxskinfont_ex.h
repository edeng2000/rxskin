...
public:
    ...
    */
	...
	sk_sp<SkTypeface> GetFont() { return m_skFont; }
    const LOGFONT& LogFont() { return m_lf; }
    public:
//字体句柄
	sk_sp<SkTypeface>  m_skFont;   //定义字体
	SkFont* m_poFont;
...
public:
	...
	int32_t GetTextWidthByFont(const LOGFONT& oFont,
		const tstring& strTxt,
		const uint32_t uFormat = DT_SINGLELINE | DT_LEFT);
	VOID    GetTextSize(const uint32_t& dwFontID,
		const tstring& strTxt,SIZE& sz,
		const uint32_t uFormat =DT_SINGLELINE|DT_LEFT);
	void    GetTextSizeByFont(const LOGFONT& oFont,
		const tstring& strTxt,
		SIZE& sz,
		const uint32_t uFormat = DT_SINGLELINE | DT_LEFT);
	...
	sk_sp<SkFontMgr> GetFontMgr() { return m_pSkFontMgr; }
