...
public:
    ...
    size_t content_size() { return contentSize; }
private:
    static bool checkIfConnection(const Response::HeaderItem &item)
    {
        return RXStrcasecmp(item.name.c_str(), "Connection") == 0;
    }
        ...
        while( begin != end )
        {
            ...
            case HeaderValue:
                if( input == '\r' )
                {
                    ...
                    if(RXStrcasecmp(h.name.c_str(), "Content-Length") == 0 )
                    {
                        contentSize = atoi(h.value.c_str());
                        resp.content.reserve( contentSize );
                    }
                    else if(RXStrcasecmp(h.name.c_str(), "Transfer-Encoding") == 0 )
                    {
                        if(RXStrcasecmp(h.value.c_str(), "chunked") == 0)
                            chunked = true;
                    }
                    state = ExpectingNewline_2;
                }
            ...
            case ExpectingNewline_3: {
                ...
                {
                    if(RXStrcasecmp(it->value.c_str(), "Keep-Alive") == 0 )
                    {
                        resp.keepAlive = true;
                    }
                    else  // == Close
