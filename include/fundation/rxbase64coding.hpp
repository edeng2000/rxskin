#ifndef _RX_BASE64_CODING_H____
#define _RX_BASE64_CODING_H____
...
static inline std::string base64_decode(std::string const& encoded_string)
{
	...
	int j = 0;
	int in_ = 0;
	unsigned char char_array_4[4] = { 0 }, char_array_3[3];
	std::string ret;
	...
	if (i) {
		...
		for (j = 0; j < 4; j++)
			char_array_4[j] = (unsigned char)base64_chars.find((char)char_array_4[j]);
